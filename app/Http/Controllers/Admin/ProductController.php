...
=======
            ...
            foreach ($all_temp_att as $att) {
                ...
                DB::table('postmeta')->insert([
                    ...
                    'meta_key' => 'attribute_stock',
                    'meta_value' => $request->stockQuality,
>>>>>>> ecb49ed0d093a68183dd08783e97cabe5f209883
                ]);
                DB::table('postmeta')->insert([
                    'post_id' => $id_last,
                    'meta_key' => 'attribute_low_stock',
...
// product edit
    public function edit($id, Request $request) {
        if ($request->user()->can('manage-product')) {
            ...
            $attributes = attribute_taxonomie::where('status', 1)->get();
            return view('admin.product.edit', compact('categories', 'tags', 'attributes', 'product', 'nameTaxonomy', 'tagTaxonomy', 'bandTaxonomy', 'image', 'stock_status', 'regular_price', 'sale_price', 'weight', 'length', 'width', 'height', 'qty', 'alert_qty', 'allAttribute', 'stock', 'sku', 'gallery_images'
                    ))->with($extraInfo);
        }
    }
